...
# Tested: works
# Counts number of white regions in the image
def getNumWhiteRegions(features):

    # Make grid containing pixel vals
    whitePixels = {}
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            if features[(x, y)] == 0:
    ...
    while len(whitePixels) > 0:
        ...
        # Set up stack
        stack = util.Stack()
        pixel = whitePixels.keys()[0]
        del(whitePixels[pixel])
        for n in getNeighbors(pixel):
            stack.push(n)

        while not stack.isEmpty():
            pixel = stack.pop()
            if pixel in whitePixels:
                del(whitePixels[pixel])
                for n in getNeighbors(pixel):
                    stack.push(n)
